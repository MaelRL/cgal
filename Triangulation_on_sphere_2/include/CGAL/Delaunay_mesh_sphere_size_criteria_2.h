// Copyright (c) 2003-2004, 2012, 2019  INRIA Sophia-Antipolis (France).
// All rights reserved.
//
...
public:
  ...
  public:
    ...
    Vertex_handle nearest_vertex(const Point_2& p, Face_handle f) const
    {
      ...
      look_nearest_neigh3bor(p, f, 1, nn);
      look_nearest_neighbor(p, f, 2, nn);
      return nn;
    }

    void look_nearest_neighbor(const Point_2& p, Face_handle f, int i,
                               Vertex_handle& nn) const
    {
      Face_handle ni = f->neighbor(i);
      if(ON_NEGATIVE_SIDE == traits.orientation_2_object()(ni->vertex(0)->point(),
                                                           ni->vertex(1)->point(),
                                                           ni->vertex(2)->point(), p))
        return;

      typename Geom_traits::Compare_distance_2 compare_distance = traits.compare_distance_2_object();
      i = ni->index(f);
                                    ...
                                    Quality& q) const
    {
      ...
      typedef typename Geom_traits::Construct_circumcenter_2     Construct_circumcenter_2;
      typedef typename Geom_traits::Construct_midpoint_2         Construct_midpoint_2;
