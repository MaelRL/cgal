...
public:
  ...
  Solid_edges_iterator finite_edges_begin() const { return DTS::solid_edges_begin(); }
  Solid_edges_iterator finite_edges_end() { return DTS::solid_edges_end(); }
  Solid_faces_iterator finite_faces_begin() { return DTS::solid_faces_begin(); }
  Solid_faces_iterator finite_faces_end() { return DTS::solid_faces_end(); }
  All_vertices_iterator finite_vertices_end() { return vertices_end(); }
  All_vertices_iterator finite_vertices_begin() { return vertices_begin(); }

  //is_infinite(...) is needed for meshing
  bool is_infinite(Vertex_handle v) const { return false; }
  bool is_infinite(Face_handle f) const { return f->is_ghost(); }
  Face_handle infinite_face() const { return DTS::_ghost; }
  ...
  Constrained_Delaunay_triangulation_sphere_2(const Self& cdt) : DTS(cdt) { }
                   ...
                   OutputItFaces out = Emptyset_iterator())
  {
    ...
    while(itedge != edges.end())
    {
      ...
      if(is_flipable(f, i))
      {
        ...
        if(less_edge(*itedge, eni))
          edge_set.insert(*itedge);
...
is_flipable(Face_handle f, int i, bool perturb) const
{
  ...
  return (power_test(ni, f->vertex(i)->point(), perturb) == ON_POSITIVE_SIDE);
}
...
{
  return DTS::geom_traits().construct_circumcenter_2_object()(p0, p1, p2);
}
       ...
       Face_handle start) const
{
  ...
  // to be able to avoid inserting a point which is too close to an existing
