...
public:
  ...
  reference operator[](key_type k) const { return stars[k]->center_point(); }
...
template<typename K, typename StarHandle>
class Kd_tree_for_star_set :
    public CGAL::Kd_tree<CGAL::Search_traits_adapter<
                                 typename Star_property_map<K, StarHandle>::Point,
                                 Star_property_map<K, StarHandle>,
                                 CGAL::Search_traits_2<K> > >
{
...
public:
  Kd_tree_for_star_set(const std::vector<StarHandle>& stars,
    ...
    Base(boost::counting_iterator<int>(0),
         boost::counting_iterator<int>(static_cast<int>(stars.size())),
