...
Scene_constrained_surface_implicit_item::
Scene_constrained_surface_implicit_item(Implicit_surface* f)
  : function_(f)
...
Scene_constrained_surface_implicit_item::~Scene_constrained_surface_implicit_item()
{
  delete frame_;
}


Scene_constrained_surface_implicit_item::Bbox
Scene_constrained_surface_implicit_item::bbox() const
{
  CGAL::Bbox_3 bb = function_->get_bbox();
  return Bbox(bb.xmin(), bb.ymin(), bb.zmin(), bb.xmax(), bb.ymax(), bb.zmax());
}

void
Scene_constrained_surface_implicit_item::direct_draw() const
{  
...
QString
Scene_constrained_surface_implicit_item::toolTip() const
{
  return tr("<p>Function <b>%1</b>").arg(this->name());
}

bool
Scene_constrained_surface_implicit_item::supportsRenderingMode(RenderingMode m) const
{ 
  switch ( m )
  {
    case Gouraud:
...
compute_function_grid()
{
  ...
  for(int i=0 ; i<grid_size_ ; ++i)
  {
    ...
    for(int j=0 ; j<grid_size_ ; ++j)
    {
      ...
      double v = function_->evaluate(query.x(), query.y(), query.z());
  ...
  for ( int i = 0 ; i <= probes_nb ; ++i )
  {
    ...
    for ( int j = 0 ; j <= probes_nb ; ++j )
    {
      ...
      for ( int k = 0 ; k <= probes_nb ; ++k )
      {
        ...
        double v = function_->evaluate(x,y,z);
...
#include "Scene_constrained_surface_implicit_item.moc"
