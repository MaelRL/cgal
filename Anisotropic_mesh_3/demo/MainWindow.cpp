  ...
  : CGAL::Qt::DemosMainWindow(parent)
{
  ...
  connect(viewer, SIGNAL(selectedPoint(double, double, double)),
          scene, SIGNAL(selectedPoint(double, double, double)));
...
void MainWindow::reload_item() {
  ...
  Io_plugin_interface* fileloader = find_loader(loader_name);
...
Io_plugin_interface* MainWindow::find_loader(const QString& loader_name) const {
  Q_FOREACH(Io_plugin_interface* io_plugin, 
            io_plugins) {
...
void MainWindow::open(QString filename)
{
  ...
  if ( dfs_it==default_plugin_selection.end() )
  {
    ...
    Q_FOREACH(Io_plugin_interface* io_plugin, io_plugins) {
      all_items << io_plugin->name();
      QStringList split_filters = io_plugin->nameFilters();
      bool stop=false;
...
Scene_item* MainWindow::load_item(QFileInfo fileinfo, Io_plugin_interface* loader) {
...
void MainWindow::on_actionLoad_triggered()
{
  ...
  typedef QMap<QString, Io_plugin_interface*> FilterPluginMap;
  FilterPluginMap filterPluginMap;
  
  Q_FOREACH(Io_plugin_interface* plugin, io_plugins) {
    QStringList split_filters = plugin->nameFilters();
    Q_FOREACH(const QString& filter, split_filters) {
  ...
  Io_plugin_interface* selectedPlugin = NULL;
