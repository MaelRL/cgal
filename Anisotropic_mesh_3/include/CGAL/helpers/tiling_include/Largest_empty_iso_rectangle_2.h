...
//
// $URL: svn+ssh://scm.gforge.inria.fr/svn/cgal/branches/releases/CGAL-3.9-branch/Largest_empty_rect_2/include/CGAL/Largest_empty_iso_rectangle_2.h $
...
public:
    ...
    typedef typename K::Point_2 Point_2_inexact;
    ...
    struct Proj_point {
        ...
        Point_2&       operator()( Node& x)       const {
            return x->p.original;
        }
        const Point_2& operator()( const Node& x) const {
            return x->p.original;
        }
    };
...
private:
    ...
    */
    ...
    void phase_1(const Point_2 query);
    // the first phase is divided to work on the x-axis and work on the y-axis
    void phase_1_on_x(const Point_2 query);
    void phase_1_on_y(const Point_2 query);
    // the second phase is divided to four parts,
    // one for each edge of the bounding box
    void phase_2_on_bot(const Point_2 query);
    void phase_2_on_top(const Point_2 query);
    void phase_2_on_left(const Point_2 query);
    void phase_2_on_right(const Point_2 query);
    void phase_2(const Point_2 query);
    void phase_3(const Point_2 query);
                                  ...
                                  const Point_2 query)
    {
        if (first_iter_is_right) {
            if (!second_iter_is_right)
                check_for_larger((*iter2)->p, (*iter)->p, (*iter1)->p, (*iter3)->p, query);
        } else
            if (second_iter_is_right)
                check_for_larger((*iter1)->p,(*iter)->p,(*iter2)->p,(*iter3)->p, query);
    }

    void empty_tents();

    // call the computation of the largest empty rectangle .
    void update(const Point_2 query);
    // init class.
        ...
        const Point_2 query)
{
    ...
    //NT rect_size = 2.0 * ( (rec.xmax() - rec.xmin()) + (rec.ymax() - rec.ymin()));
    NT rect_size = rec.area();

    if (!rec.has_on_bounded_side(query))
    {
        rect_size = 0.0;
        return;
    }
...
template<class T>
void
Largest_empty_iso_rectangle_2<T>::phase_1_on_x(const Point_2 query)
{
    ...
    while (iter != last_iter) {
        ...
        if ((*iter)->type != TOP_RIGHT && (*iter)->type != TOP_LEFT) {
            check_for_larger((*prev_iter)->p, bl_p, (*iter)->p, tr_p, query);
            prev_iter = iter;
        }
        ++iter;
    }
...
template<class T>
void
Largest_empty_iso_rectangle_2<T>::phase_1_on_y(const Point_2 query)
{
    ...
    while (iter != last_iter) {
        ...
        if ((*iter)->type != BOT_RIGHT && (*iter)->type != TOP_RIGHT) {
            check_for_larger(bl_p, (*prev_iter)->p, tr_p, (*iter)->p, query);
            prev_iter = iter;
        }
        ++iter;
    }
...
template<class T>
void
Largest_empty_iso_rectangle_2<T>::phase_1(const Point_2 query)
{
    phase_1_on_x(query);
    phase_1_on_y(query);
}
...
template<class T>
void
Largest_empty_iso_rectangle_2<T>::phase_2_on_bot(const Point_2 query)
{
    ...
    while (size - 4 > points_removed && iter3 != Point_data_list.end()) {
        if (less_yx(*iter1, *iter2) && larger_yx(*iter2, *iter3)) {
            check_for_larger((*iter1)->p, bl_p, (*iter3)->p, (*iter2)->p, query);
...
template<class T>
void
Largest_empty_iso_rectangle_2<T>::phase_2_on_top(const Point_2 query)
{
    std::list<Point_data *> Point_data_list;
    std::copy(x_sorted.begin(),
              x_sorted.end(),
    ...
    while (size - 4 > points_removed && iter3 != Point_data_list.end()) {
        if (larger_yx(*iter1, *iter2) && less_yx(*iter2, *iter3)) {
            check_for_larger((*iter1)->p,tr_p, (*iter3)->p,(*iter2)->p, query);
            ++points_removed;
            Point_data_list.erase(iter2);
            if (iter1 != first_iter) {
                iter2 = iter1;
...
template<class T>
void
Largest_empty_iso_rectangle_2<T>::phase_2_on_left(const Point_2 query)
{
    std::list<Point_data *> Point_data_list;
    ...
    while (size - 4 > points_removed && iter3 != Point_data_list.end()) {
        if (less_xy(*iter1, *iter2) && larger_xy(*iter2, *iter3)) {
            check_for_larger(bl_p, (*iter1)->p, (*iter2)->p, (*iter3)->p, query);
            ++points_removed;
            Point_data_list.erase(iter2);
            if (iter1 != first_iter) {
                iter2 = iter1;
...
template<class T>
void
Largest_empty_iso_rectangle_2<T>::phase_2_on_right(const Point_2 query)
{
    std::list<Point_data *> Point_data_list;
    std::copy(y_sorted.begin(),
              y_sorted.end(),
    ...
    while (size - 4 > points_removed && iter3 != Point_data_list.end()) {
        if (larger_xy(*iter1, *iter2) && less_xy(*iter2, *iter3)) {
            check_for_larger((*iter2)->p, (*iter1)->p, tr_p, (*iter3)->p, query);
            ++points_removed;
            Point_data_list.erase(iter2);
...
template<class T>
void
Largest_empty_iso_rectangle_2<T>::phase_2(const Point_2 query)
{
    phase_2_on_top(query);
    phase_2_on_left(query);
    phase_2_on_right(query);

    // Done only for building tents for phase 3
    phase_2_on_bot(query);
}


template<class T>
void
Largest_empty_iso_rectangle_2<T>::phase_3(const Point_2 query)
{
...
template<class T>
void
Largest_empty_iso_rectangle_2<T>::update(const Point_2 query)
{
    if (! cache_valid) {
        largest_rect_size = 0;
...
template<class T>
typename Largest_empty_iso_rectangle_2<T>::Iso_rectangle_2
Largest_empty_iso_rectangle_2<T>::get_largest_empty_iso_rectangle(const Point_2 query)
{
    if (x_sorted.size() == 4) {
