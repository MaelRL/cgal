...
template <typename TriangleMesh>
bool
locate_in_common_face(typename internal::Location_traits<TriangleMesh>::Face_location& known_location,
                      const typename internal::Location_traits<TriangleMesh>::Point& query,
                      typename internal::Location_traits<TriangleMesh>::Face_location& query_location,
                      const TriangleMesh& tm,
                      const typename internal::Location_traits<TriangleMesh>::FT tolerance =
                        std::numeric_limits<typename internal::Location_traits<TriangleMesh>::FT>::epsilon())
{
  typedef typename boost::graph_traits<TriangleMesh>::vertex_descriptor    vertex_descriptor;
  typedef typename boost::graph_traits<TriangleMesh>::halfedge_descriptor  halfedge_descriptor;
  typedef typename boost::graph_traits<TriangleMesh>::face_descriptor      face_descriptor;
  ...
  if(const vertex_descriptor* vd_ptr = boost::get<vertex_descriptor>(&dv))
  {
    ...
    for(face_descriptor fd : CGAL::faces_around_target(hd, tm))
    {
      if(fd == boost::graph_traits<TriangleMesh>::null_face())
        continue;
      ...
      internal::snap_location_to_border(query_location, tm, tolerance); // @tmp keep or not ?
