                                    ...
                                    const NamedParameters& np = parameters::default_values())
{
  ...
  // Needless-ness and cap-ness is checked again at pop time anyway, so pointless to compute it here
  for(face_descriptor f : face_range)
  ...
  for(;;)
  {
    ...
    {
      // note that on the first iteration, 'h' is meaningless
      ...
      {
#ifdef CGAL_PMP_DEBUG_REMOVE_DEGENERACIES_EXTRA
        std::cout << "\t Needle criterion no longer verified for " << h << std::endl;
#endif
      ...
      if(CGAL::Euler::does_satisfy_link_condition(e, tmesh))
      {
        ...
        for(halfedge_descriptor hv : halfedges_around_target(v, tmesh))
        {
          ...
          {
            next_edges_to_collapse.insert(hv); // it will be actually tested when popped
          }
        }
    ...
    while(!edges_to_flip.empty())
    {
      ...
      {
#ifdef CGAL_PMP_DEBUG_REMOVE_DEGENERACIES_EXTRA
        std::cout << "\t Cap criterion no longer verified for " << h << std::endl;
#endif
        continue;
      }
      else
