...
#ifndef CGAL_POLYGON_MESH_PROCESSING_SNAPPING_SNAP_EDGES_H
#define CGAL_POLYGON_MESH_PROCESSING_SNAPPING_SNAP_EDGES_H
...
{
  // @todo The simplification below is too naive and will fail to treat complicated cases.
  //
  // For example, a border range with zigzaging long edges, a better simplification
  // would create a fuzzy area around the border (say the dilation
...
public:
  explicit Edge_projection_traits(const halfedge_descriptor h,
...
// #define CGAL_PMP_SNAP_USE_ANISOTROPIC_DISTANCE
  ...
  void intersection(const Point& query, const Primitive& primitive)
  {
    ...
    // We are searching for a point on the target halfedge h that is close to target(h).
                          ...
                          const GT& gt)
{
  ...
  typedef internal::Edge_projection_traits<TriangleMesh, VPMS, VPMT, FacePatchMap_T, AABB_traits> Projection_traits;
  ...
  // Get a canonical halfedge
  if(is_border(edge(closest_h, tm_T), tm_T))
  {
  ...
  {
    if(opposite(closest_h, tm_T) < closest_h)
      closest_h = opposite(closest_h, tm_T);
  }
...
template <typename TriangleMesh, typename VPMT, typename GeomTraits>
void split_border_edge(typename boost::graph_traits<TriangleMesh>::halfedge_descriptor& h_to_split,
                       const typename boost::graph_traits<TriangleMesh>::vertex_descriptor new_v,
                       TriangleMesh& tm_T,
                       VPMT vpm_T,
                       const GeomTraits& gt)
{
...
template <typename TriangleMesh, typename VPMT, typename GeomTraits>
void split_edge(typename boost::graph_traits<TriangleMesh>::halfedge_descriptor& h_to_split,
                const typename boost::graph_traits<TriangleMesh>::vertex_descriptor new_v,
                TriangleMesh& tm_T,
                VPMT vpm_T,
                const GeomTraits& gt)
{
  ...
  if(is_border(h_to_split, tm_T))
    return split_border_edge(h_to_split, new_v, tm_T, vpm_T, gt);

  halfedge_descriptor h_to_split_opp = opposite(h_to_split, tm_T);

  CGAL::Euler::split_face(prev(h_to_split, tm_T), next(h_to_split, tm_T), tm_T);
  CGAL::Euler::split_face(h_to_split_opp, prev(prev(h_to_split_opp, tm_T), tm_T), tm_T);
}
                        ...
                        const bool is_source_mesh_fixed) // when snapping is B --> A and the mesh B is fixed
{
  ...
  for(Edge_and_splitters& es : edges_to_split)
  {
    ...
    if(splitters.size() > 1)
    {
      ...
      // Multiple points splitting the same halfedge, so we sort them according to their projection,
      // so that we can just split iteratively from left to right
    ...
    for(const Vertex_with_new_position& vnp : splitters)
    {
      ...
      // Some splits can create degenerate faces (e.g. two verticeshaving the same projection), avoid that
      if((new_position == get(vpm_T, target(h_to_split, tm_T))) || (new_position == get(vpm_T, source(h_to_split, tm_T))))
        do_split = false;
      ...
      internal::split_edge(h_to_split, new_v, tm_T, vpm_T, gt);
    }
  }
...
// Collect vertices that can be projected onto an edge of the range
//
// If multiple vertices in the source range have the same geometric position, they are moved
// all at once and we use the smallest tolerance from all source vertices with that position.
// This is done to solve situations such as:
//          ||
//          ||
//      ____||____
//      ----------
...
std::size_t snap_vertex_edge_one_way(const HalfedgeRange_S& halfedge_range_S,
                                     TriangleMesh& tm_S,
...
std::size_t snap_vertex_edge_two_way(HalfedgeRange_A& halfedge_range_A,
                                     TriangleMesh& tm_A,
                                     ToleranceMap_A tolerance_map_A,
                                     HalfedgeRange_B& halfedge_range_B,
                                     TriangleMesh& tm_B,
                                     ...
                                     const NamedParameters_B& np_B)
{
  ...
  snapped_n += internal::snap_vertex_edge_one_way<ConcurrencyTag>(
                         ...
                         const NamedParameters_B& np_B)
{
  ...
  return internal::snap_vertex_edge_two_way<ConcurrencyTag>(border_vertices_A, tm_A, tolerance_map_A,
                         ...
                         const CGAL_PMP_NP_CLASS& np)
{
  ...
  return internal::snap_vertex_edge_two_way<ConcurrencyTag>(border_vertices, tm, tolerance_map,
...
#endif // CGAL_POLYGON_MESH_PROCESSING_SNAPPING_SNAP_EDGES_H
