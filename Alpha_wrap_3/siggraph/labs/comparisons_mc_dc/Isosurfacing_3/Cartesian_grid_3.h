...
namespace CGAL {
    ...
    class Cartesian_grid_3 {
      ...
      public:
        ...
        // x position of grid origin
        float offset_x() const { return grid.tx(); }
        // y position of grid origin
        float offset_y() const { return grid.ty(); }
        // z position of grid origin
    ...
    template<typename T>
    void Cartesian_grid_3<T>::create_image( const std::size_t xdim, const std::size_t ydim, const std::size_t zdim, const Bbox_3& bbox ) {
        WORD_KIND wordkind;
