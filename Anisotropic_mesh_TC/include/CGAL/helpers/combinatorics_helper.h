...
public :
  ...
  Ordered_simplex_base(const Iter b, const Iter e)
  {
    assert(e-b == ssize);
    std::copy(b, e, m_simplex.begin());
    sort();
  }
};
...
struct Simplex_unordered_set
{
  typedef boost::unordered_set<Ordered_simplex_base<ssize>,
                               Ordered_simplex_base_hash<ssize>,
                               Ordered_simplex_base_comparer<ssize> > type;
};
